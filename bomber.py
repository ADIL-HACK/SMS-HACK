...
def do_zip_update():
    ...
    if success:
        mesgdcrt.SuccessMessage("SMS BOMB was updated to the latest version")
        mesgdcrt.GeneralMessage(
            "Please run the script again to load the latest version")
    else:
        mesgdcrt.FailureMessage("Unable to update SMS BOMB.")
...
def check_for_updates():
    ...
    if fver != __VERSION__:
        ...
        mesgdcrt.SuccessMessage("SMS BOMB is up-to-date")
        mesgdcrt.GeneralMessage("Starting SMS BOMB")
...
def pretty_print(cc, target, success, failed):
    ...
    mesgdcrt.SuccessMessage("SMS BOMB was created by ADIL")
...
if sys.version_info[0] != 3:
    mesgdcrt.FailureMessage("SMS BOMB will work only in Python v3")
    sys.exit()
...
__CONTRIBUTORS__ = ['ADIL', 't0xic0der', 'scpketer', 'Stefan']
...
description = """SMS BOMB - Your Friendly Spammer Application

SMS BOMB can be used for many purposes which incudes -
\t Exposing the vulnerable APIs over Internet
\t Friendly Spamming
\t Testing Your Spam Detector and more ....

SMS BOMB is not intented for malicious uses.
"""

parser = argparse.ArgumentParser(description=description,
                                 epilog='Coded by ADIL !!!')
parser.add_argument("-sms", "--sms", action="store_true",
                    help="start SMS BOMB with SMS Bomb mode")
parser.add_argument("-call", "--call", action="store_true",
                    help="start SMS BOMB with CALL Bomb mode")
parser.add_argument("-mail", "--mail", action="store_true",
                    help="start SMS BOMB with MAIL Bomb mode")
parser.add_argument("-ascii", "--ascii", action="store_true",
                    help="show only characters of standard ASCII set")
parser.add_argument("-u", "--update", action="store_true",
                    help="update SMS BOMB")
parser.add_argument("-c", "--contributors", action="store_true",
                    help="show current SMS BOMB contributors")
parser.add_argument("-v", "--version", action="store_true",
                    help="show current SMS BOMB version")
